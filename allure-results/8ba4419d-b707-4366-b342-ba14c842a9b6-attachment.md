...
```
Error: locator.fill: Unsupported token "@id" while parsing css selector "/input[@id="user-name"]". Did you mean to CSS.escape it?
Call log:
  - waiting for /input[@id="user-name"]
...
```ts
  ...
  8  |     }
  9  |     //login method
  10 |     async login(uname, pword) {
> 11 |         await this.page.locator(this.username).fill(uname);
     |                                                ^ Error: locator.fill: Unsupported token "@id" while parsing css selector "/input[@id="user-name"]". Did you mean to CSS.escape it?
  12 |         await this.page.locator(this.password).fill(pword);
